...
def main():
    ...
    write_to_gdoc(mytelecom_inst) if mytelecom_inst else print(f'{mytelecom.__name__} Error')
    ...
    write_to_gdoc(kyivstar_inst) if kyivstar_inst else print(f'{kyivstar.__name__} Error')

    # Ukrtelecom - Купянск
    ukrtelecom_inst = get_parser_instance(ukrtelecom, coordinates=[1363, 1384, 10639, 'Купянск'])
    write_to_gdoc(ukrtelecom_inst) if ukrtelecom_inst else print(f'{ukrtelecom.__name__} Error')

    # Ukrtelecom - Купянск-Узловой
    ukrtelecom_inst = get_parser_instance(ukrtelecom, coordinates=[1363, 1384, 10640, 'Купянск-Узловой'])
    write_to_gdoc(ukrtelecom_inst) if ukrtelecom_inst else print(f'{ukrtelecom.__name__} Error')

    # Ukrtelecom - Ковшаровка
    ukrtelecom_inst = get_parser_instance(ukrtelecom, coordinates=[1363, 1384, 10625, 'Ковшаровка'])
    write_to_gdoc(ukrtelecom_inst) if ukrtelecom_inst else print(f'{ukrtelecom.__name__} Error')

    # Ukrtelecom - Подолы
    ukrtelecom_inst = get_parser_instance(ukrtelecom, coordinates=[1363, 1384, 14265, 'Подолы'])
    write_to_gdoc(ukrtelecom_inst) if ukrtelecom_inst else print(f'{ukrtelecom.__name__} Error')

    # Ukrtelecom - Петропавловка
    ukrtelecom_inst = get_parser_instance(ukrtelecom, coordinates=[938, 960, 9168, 'Петропавловка'])
    write_to_gdoc(ukrtelecom_inst) if ukrtelecom_inst else print(f'{ukrtelecom.__name__} Error')

    # Ukrtelecom - Куриловка
    ukrtelecom_inst = get_parser_instance(ukrtelecom, coordinates=[938, 959, 12412, 'Куриловка'])
    write_to_gdoc(ukrtelecom_inst) if ukrtelecom_inst else print(f'{ukrtelecom.__name__} Error')

    # Ukrtelecom - Куриловка
    kupmytelecom_inst = get_parser_instance(kupmytelecom)
    write_to_gdoc(kupmytelecom_inst) if kupmytelecom_inst else print(f'{kupmytelecom.__name__} Error')
