...
def main():
    ...
    write_to_gdoc(blacksea_inst) if blacksea_inst else print(
        f"{blacksea.__name__} Error"
    )
    ...
    write_to_gdoc(lionline_inst) if lionline_inst else print(
        f"{lionline.__name__} Error"
    )
    ...
    ukrtelecom_inst = get_parser_instance(
        ukrtelecom, coordinates=[1249, 1275, 10179, "Одесса"]
    )
    ...
    # Intertelecom
    intertelecom_inst = get_parser_instance(
        intertelecom, coordinates=[1249, 1275, 10179, "Одесса"]
    )
    write_to_gdoc(intertelecom_inst) if intertelecom_inst else print(
        f"{intertelecom.__name__} Error"
    )
    ...
    write_to_gdoc(kyivstar_inst) if kyivstar_inst else print(
        f"{westkyivstarele.__name__} Error"
    )

    # Vodafone
    vodafone_inst = get_parser_instance(vodafone)
    write_to_gdoc(vodafone_inst) if vodafone_inst else print(
        f"{vodafone.__name__} Error"
    )
