...
class Prostor(DinamicParser):
    ...
    def parse(self, city, competitor):
        ...
        sleep(20)
        try:
            close = browser.find_element_by_xpath('//button[@class="sendpulse-prompt-close"]').click()	
        ...
        sleep(5)
        try:
            close = browser.find_element_by_xpath('//span[@class="b24-widget-button-popup-btn-hide"]').click()	
        except Exception as e:
            pass		
        sleep(pr_delay)
        pick = browser.find_element_by_xpath('//h5[@class="town-text-pick"]//a').click()
        sleep(pr_delay)
        ul = browser.find_element_by_xpath('//ul[@class="region-items-wrap"]//li[1]').click()
        sleep(pr_delay)
